<template>
  <s-button-group class="adar-app-controls-group">
    <app-account-button :disabled="loading" @click="goTo(PageNames.Wallet)" class="no-box-shadow" />
    <template v-if="!isLargeDesktop">
      <s-button type="action" :tooltip="t('headerMenu.settings')" @click="handleClickHeaderMenu">
        <s-dropdown
          ref="headerMenu"
          class="adar-header-menu__button"
          popper-class="adar-header-menu"
...
.el-dropdown-menu__item.adar-header-menu__item.is-disabled {
...
$adar-app-controls-filter: drop-shadow(-5px -5px 5px rgba(232, 25, 50, 0.05))
  drop-shadow(1px 1px 25px rgba(232, 25, 50, 0.1));
$adar-app-controls-shadow: inset 1px 1px 10px #ffffff;

$adar-app-controls-filter--dark: drop-shadow(-5px -5px 5px rgba(232, 25, 50, 0.05))
  drop-shadow(2px 2px 10px rgba(232, 25, 49, 0.33));
$adar-app-controls-shadow--dark: inset 1px 1px 2px #52523d;
.no-box-shadow {
